...
void main() {
  ...
  // Portal colors - deep blues and purples
  vec3 deepBlue = vec3(0.1, 0.2, 0.8);
  vec3 purple = vec3(0.4, 0.1, 0.8);
  vec3 darkBlue = vec3(0.05, 0.1, 0.4);
  
  // Mix colors based on spiral layers
  vec3 color = mix(deepBlue, purple, spiral);
  color = mix(color, darkBlue, spiral2 * 0.4);
  color = mix(color, vec3(0.2, 0.3, 1.0), spiral3 * 0.3);
